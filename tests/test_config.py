...
def test_config_is_frozen_slotted_and_omits_secrets_from_repr(tmp_path):
    ...
    for sentinel in (provider_secret, api_secret, extra_secret):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("pdf_pages", (1,)),
        ("pdf_password", "secret"),
        ("pdf_allow_partial", True),
    ],
)
def test_config_does_not_expose_unimplemented_pdf_options(field_name, value):
    assert not hasattr(Config(), field_name)
    with pytest.raises(TypeError):
        Config(**{field_name: value})  # type: ignore[arg-type]
...
def test_config_validation_error_does_not_render_other_secret_fields():
    ...
    with pytest.raises(ConfigError) as caught:
        Config(
            ...
            extra={"provider_token": secrets[1]},
            timeout_seconds=0,
        )
...
def test_config_normalizes_ordered_language_sequences_and_timeout():
    config = Config(
...
def test_config_rejects_invalid_language_values(kwargs):
