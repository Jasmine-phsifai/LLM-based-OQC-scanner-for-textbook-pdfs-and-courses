...
def test_plain_import_binds_recognition_facades_but_defers_execution_helpers():
    ...
    probe = (
        ...
        "extract_pdf_pages=ocrllm.extract_pdf_pages; "
        ...
        "repair_images=ocrllm.repair_images_to_markdown; "
        "repair_audio=ocrllm.repair_audio_to_markdown; "
        "restore_image_plan=ocrllm.restore_image_batch_plan; "
        "batchify_module=importlib.import_module('ocrllm.batchify_images'); "
        "assert callable(batchify_images); "
        "assert callable(extract_pdf_pages); "
        ...
        "assert callable(repair_images); "
        "assert callable(repair_audio); "
        "assert callable(restore_image_plan); "
        "assert 'ocrllm.output.load_merged_image_resume_state' not in sys.modules; "
