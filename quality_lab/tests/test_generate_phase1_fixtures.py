...
from quality_lab.generators.generate_phase1_fixtures import (
    GENERATED_FILENAMES,
...
from quality_lab.generators.load_quality_font import (
    FONT_BYTES,
    FONT_SHA256,
)
...
PHASE1_ROOT = REPOSITORY_ROOT / "quality_lab" / "fixtures" / "phase1"
IMAGE_DIRECTORY = PHASE1_ROOT / "images"
FONT_DIRECTORY = REPOSITORY_ROOT / "quality_lab" / "assets" / "fonts"
GENERATOR_DIRECTORY = REPOSITORY_ROOT / "quality_lab" / "generators"
...
EXPECTED_SUPPORT_ARTIFACTS = {
    ...
    "provenance.json": (
        996,
        "e11e0d7d72789196e34ad2cf11d08285ea95c39d0a35fcff9c068c0753127bff",
    ),
}
...
def _run_generator_cli(*arguments: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [
            sys.executable,
            "-m",
            "quality_lab.generators.generate_phase1_fixtures",
            *arguments,
        ],
        cwd=REPOSITORY_ROOT,
        text=True,
...
def test_font_license_and_provenance_are_hash_pinned() -> None:
    ...
    assert provenance["artifact_path"] == (
        "quality_lab/assets/fonts/NotoSansCJKsc-Regular.otf"
    )
    assert provenance["license_spdx"] == "OFL-1.1"
    assert provenance["license_path"] == "quality_lab/assets/fonts/OFL-1.1.txt"
...
def test_whiteboard_provenance_matches_committed_derivative() -> None:
    ...
    assert _sha256(provenance_path) == (
        "74dce271a790ef6d50418bb0117787428d87dad835ac7f17d9a82e95cc09ad96"
    )
    provenance = json.loads(provenance_path.read_text(encoding="utf-8"))
    ...
    assert provenance["fixture_path"] == (
        "quality_lab/fixtures/phase1/images/handwritten_whiteboard.jpg"
    )
