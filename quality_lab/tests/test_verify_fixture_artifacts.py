...
from quality_lab.fixture_manifest import ArtifactRecord, LicenseRecord
from quality_lab.load_fixture_manifest import load_fixture_manifest
from quality_lab.verify_fixture_artifacts import (
...
def test_verifies_every_hash_image_license_and_provenance_offline() -> None:
    ...
    assert report.total_bytes == 17_887_107 + manifest.raw_bytes
...
def test_rejects_symlinked_artifact_without_following_it(
    ...
    root = tmp_path / "repo"
    lab_dir = root / "quality_lab"
    lab_dir.mkdir(parents=True)
    link = lab_dir / "linked.txt"
...
def test_rejects_license_text_that_does_not_match_spdx(tmp_path: Path) -> None:
    root = tmp_path / "repo"
    license_path = root / "quality_lab" / "license.txt"
...
def test_rejects_undeclared_file_in_any_frozen_corpus_root(tmp_path: Path) -> None:
    ...
    manifest_path = source_root / "quality_lab" / "fixtures" / "phase1" / "manifest.json"
    target_manifest = root / "quality_lab" / "fixtures" / "phase1" / "manifest.json"
    ...
    extra = root / "quality_lab" / "generators" / "undeclared.py"
...
def test_rejects_manifest_drift_after_loading(tmp_path: Path) -> None:
    ...
    for artifact in manifest.artifacts:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(source, target)
    manifest_path = root / "quality_lab" / "fixtures" / "phase1" / "manifest.json"
    manifest_path.parent.mkdir(parents=True, exist_ok=True)
    manifest_path.write_bytes(
        (source_root / "quality_lab" / "fixtures" / "phase1" / "manifest.json").read_bytes()
        + b"\n"
    )
