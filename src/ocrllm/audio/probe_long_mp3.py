...
def probe_long_mp3(snapshot_path: Path) -> float:
    """Return decoded seconds inside the current one-file Google A2a envelope."""
    ...
    if duration_seconds > MAX_GOOGLE_FILES_AUDIO_DURATION_SECONDS:
        raise InvalidSource(
            "The MP3 source exceeds the Google single-prompt audio limit.",
