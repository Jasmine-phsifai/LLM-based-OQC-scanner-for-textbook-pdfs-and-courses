...
_FAILURE_SECTION = re.compile(
    ...
    rf"(?: actual=(?P<actual_start>{_SECONDS})-"
    rf"(?P<actual_end>{_SECONDS})s)? -->)$",
    re.MULTILINE,
)
...
def parse_merged_audio_failure_markers(
    ...
    *,
    ...
    for match in matches:
        ...
        raw_actual_start = match.group("actual_start")
        raw_actual_end = match.group("actual_end")
        if raw_actual_start is None or raw_actual_end is None:
            actual_start = max(
                0.0,
            ...
            actual_start = float(_parse_seconds(raw_actual_start))
            actual_end = float(_parse_seconds(raw_actual_end))
        if not (
            0.0 <= actual_start <= logical_start < logical_end <= actual_end
            <= float(duration_seconds)
        ):
            _raise_invalid_markdown()
        markers.append(
