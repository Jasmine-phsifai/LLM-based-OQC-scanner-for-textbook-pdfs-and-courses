...
|---|---:|---|
| Base | Wheel <= 320 KiB; no-deps target <= 1.5 MiB | Zero base runtime requirements, zero native binaries, no PIL/pypdfium2/openai/httpx imports; target bytes retain installer-generated bytecode for a real disk measurement. 30 measured fresh processes after two warm-ups. Wall median <= 100 ms and p95 <= 200 ms; process-CPU median <= 60 ms and p95 <= 100 ms. Record maxima diagnostically. |
...
$wheel = Get-ChildItem -LiteralPath $wheelDir -Filter *.whl | Select-Object -First 1
if ($null -eq $wheel) { throw "wheel build produced no wheel" }
if ($wheel.Length -gt 327680) { throw "Base wheel exceeds 320 KiB: $($wheel.Length)" }
& D:\Anaconda\envs\OCRLLM\python.exe -m pip install --no-deps --target $targetDir $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "isolated wheel install failed" }
$installedBytes = (Get-ChildItem -LiteralPath $targetDir -Recurse -File | Measure-Object Length -Sum).Sum
if ($installedBytes -gt 1572864) { throw "Base target exceeds 1.5 MiB: $installedBytes" }
