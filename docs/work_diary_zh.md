...
**过度设计复查。** 没有自动选模、模型 sweep、quality replay、provider class、credential pool、retry、fallback 或 runtime usage 改造。model-attempt 与 workflow-slot 两次校验不是重复防御：前者排除 candidate switch，后者排除 review/scout，直接证明 one-call contract。
